...
class Agent:
    ...
    def __init__(self):
        ...
        self.model = Linear_QNet(129, 512, 3)
    ...
    def get_state(self, game):
        ...
        state = []
        vision = game.get_dirs(grid_size=10)

        for vx, vy in vision:
            if vx < 0 or vx >= game.w or vy < 0 or vy >= game.h:
                state.append(-1)
            elif [vx, vy] in game.body[1:]:
                state.append(-1)
            elif [vx, vy] == game.food:
                state.append(1)
            else:
                state.append(0)
